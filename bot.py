...
import httpx
import aiohttp
# ── Global Proxy Cooldown Setup ─────────────────────────────────────────────
USER_PROXY_COOLDOWN = {}
COOLDOWN_24H = 86400  # 24 hours in seconds
...
TOKEN = "8604794001:AAENyv_KH8O5Vk9zKe6To-qf9Zdez9FM5Ow" 
...
async def cmd_start(message: types.Message):
    ...
    if not joined:
        await message.reply(
            JOIN_MSG,
            reply_markup=menu_keyboard(),  # ✅ دوانەیی کەوان() هاتنە زێدەکرن
        )
        return
...
async def cmd_sh(message: types.Message):
    joined = await check_user_joined(message.from_user.id)
    if not joined:
        await message.reply(JOIN_MSG, reply_markup=board())  # ✅ گۆڕی بۆ board()
        return

    # بەرنامە دێ لێرە بەردەوام بێت بۆ چیک کرنا کارتی...
...
# ══════════════════════════════════════════════════════════════════════════════
async def check_proxy_live(session: aiohttp.ClientSession, proxy: str) -> str | None:
    """Checks if an HTTP proxy is live."""
...
async def cmd_freeproxy(message: types.Message):
    ...
    last_fetch = USER_PROXY_COOLDOWN.get(uid, 0)
    time_passed = current_time - last_fetch

    if time_passed < COOLDOWN_24H:
        remaining_seconds = int(COOLDOWN_24H - time_passed)
        hours = remaining_seconds // 3600
        minutes = (remaining_seconds % 3600) // 60
        await message.reply(
            f"{pe(E['warn'])} {bold('Cooldown Active!')}\n\n"
            f"You can fetch free proxies again in: {bold(f'{hours}h {minutes}m')}"
        )
        return

    loading = await message.reply(
        f"{pe(E['loading'])} {bold('Fetching & testing live proxies... Please wait!')}"
    )
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=10) as resp:
                ...
                raw_proxies = [p.strip() for p in text.splitlines() if p.strip()]
        ...
        async with aiohttp.ClientSession() as check_session:
            for i in range(0, len(raw_proxies), 50):
                ...
                tasks = [check_proxy_live(check_session, p) for p in batch]
                results = await asyncio.gather(*tasks)
                for p in results:
                    if p and p not in live_proxies:
        ...
        if not live_proxies:
            await safe_edit(loading, f"{pe(E['cross'])} {bold('No working live proxies found. Try again!')}")
            return

        # Cooldown update
        USER_PROXY_COOLDOWN[uid] = current_time

        # Transform proxies into dictionary format for the Callback function
        parsed_proxies = []
        for p in live_proxies:
            parts = p.split(":")
            parsed_proxies.append({"ip": parts[0], "port": parts[1] if len(parts) > 1 else ""})

        # Save into pending store
        _freeproxy_pending[uid] = parsed_proxies

        # Format display text
        proxy_lines = [f"{i}. <code>{p['ip']}:{p['port']}</code>" for i, p in enumerate(parsed_proxies, 1)]
        res_text = (
            f"{pe(E['check'])} {bold(f'Fetched {len(parsed_proxies)} Tested Live Proxies:')}\n\n"
            + "\n".join(proxy_lines) + "\n\n"
        ...
        await safe_edit(loading, res_text, reply_markup=kb)
...
async def cb_freeproxy_add(callback: types.CallbackQuery):
    ...
    # پڕۆکسیێن جارا دوماهیێ بپارێزە ل جە `_save_freeproxy_last`
    _save_freeproxy_last(uid, to_add)

    # پڕۆکسیان زێدە بکه بۆ ناڤا هەژمارا بەکارهێنەری
    existing = get_user_proxies(uid) or []
    slots_free = MAX_PROXIES_PER_USER - len(existing)

    if slots_free > 0:
        add_user_proxies(uid, to_add[:slots_free])
    ...
    lines = [
        ...
        "",
        f"{pe(E['link'])} {bold('Proxies (ip:port)')}",
        "",
    ]
    for i, proxy_str in enumerate(copy_lines, 1):
        lines.append(f"  {bold(str(i))}. <code>{proxy_str}</code>")
...
async def cb_freeproxy_copy(callback: types.CallbackQuery):
    ...
    if pending:
        for p in pending:
            # Handle both dict objects and raw string formats safely
            if isinstance(p, dict):
                ip   = p.get("ip", "?")
                port = str(p.get("port", "?"))
                user = p.get("username") or ""
        ...
        raw_saved = _load_freeproxy_last(owner_id)
        if raw_saved:
    ...
    if not lines:
        await callback.answer(bold("No saved proxies found. Run /freeproxy again."), show_alert=True)
        return

    # Send plain formatted proxy list for easy copy-pasting
    proxy_block = "\n".join(f"<code>{l}</code>" for l in lines)
    await callback.message.reply(
        f"{pe(E['link'])} {bold('All Proxies (Copy Friendly):')}\n\n{proxy_block}"
    )
    await callback.answer()
...
async def cmd_freeproxylist(message: types.Message):
    ...
    # پشکنین بۆ هەردوو جۆرێن هەڵگرتنێ (Saved database یان فایلا دوماهیێ)
    raw_saved = _load_freeproxy_last(uid) or get_user_proxies(uid) or _freeproxy_pending.get(uid)

    if not raw_saved:
        await message.reply(
            f"{pe(E['warn'])} {bold('No saved proxies found!')}\n\n"
            f"{pe(E['next'])} {bold('Run /freeproxy to fetch 30 live proxies.')}"
        )
        return
    ...
    for item in raw_saved:
        if isinstance(item, dict):
            ...
            copy_lines.append(item)

    lines = [
        f"{pe(E['link'])} {bold(f'Your Saved Proxies ({len(copy_lines)}):')}",
        "",
    ]
    for i, proxy_str in enumerate(copy_lines, 1):
        lines.append(f"  {bold(str(i))}. <code>{proxy_str}</code>")
...
async def cmd_sh(message: types.Message):
    ...
    if not auth.has_premium_access(user_id, message.chat.id):
        await message.reply(
            ...
            f"\n\n{pe(E['bolt'])} {bold('Contact admin or redeem a key: @Mod_By_BINAR')}"
            f"\n{pe(E['next'])} /redeem {bold('BINAR-xxxxx')}"
        )
        return
...
async def cmd_redeem(message: types.Message):
    ...
    if len(args) < 2 or not args[1].strip():
        await message.reply(
            f"{pe(E['warn'])} {bold('Usage:')} /redeem {bold('BINAR-xxxxx')}"
        )
        return
